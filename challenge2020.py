...
def addElements(listx3):
    ...
    return (listx3[0] + listx3[1] + listx3[2]) % 10000


def sortElements(listx3, new_element):
    listx3[0] = listx3[1]
    listx3[1] = listx3[2]
...
def saveArchive(set_listx3):
    ...
    archive.write(
        "Block start repeating 16291951775 times every 124000 iterations \n")
    archive.write(set_listx3)
    archive.write("\n End of repeating block \n \n \n")
    archive.close()


def aggregteListx3(set_listx3, listx3):
...
def main():
    ...
    # The second condition is a protection in case no repetitions are found
    while no_repetitions or iterations > iterations_max_control:
        iterations += 1
        new_element = addElements(listx3)
        listx3 = sortElements(listx3, new_element)
    ...
    if not no_repetitions:
        print("\nThe cycle was found to repeat " + str(repetitions_number) +
            " times every " + str(iterations) + " iterations\n")
        ...
        # It starts at 3 because the first three positions are occupied by the elements of the listx3 --> [1,2,3]
        for i in range(3, remaining_iterations):
            iterations += 1
            new_element = addElements(listx3)
            listx3 = sortElements(listx3, new_element)
            set_listx3 = set_listx3+str(listx3)

        aggregteListx3(set_listx3, listx3)  # The final block is added that is not repeated

        print("\nThe last four digits of the sum in the position " +
            str(last_position) + " are: ", new_element)
        ...
        print("\nRepeated cycles were not found in " + str(iterations_max_control) +
