...
while ciclos_no_def or ciclos>demasiados_ciclos: # La segunda condición es una proteccion por si no se hallan repeticiones 
    ciclos+=1
    suma= sumar(lista)%10000 # Se utilizan solo los 4 últimos dígitos
    lista = ordenar(lista)
...
# Una vez encotrado el bloque que se repite, se lo guarda, se descartan los próximos bloques repetidos, se resetea la lista a [1,2,3] y se sigue hasta el final de la secuencia. 
    ...
    print("\nLos cuatro últimos dígitos de la suma en la posición " + str(posision_final) + "\nson: ", suma)

    print("\nEl boque final de agregó al archivo salida_lista.txt \n")

    tfinal=time.time()

    tiempo=round(tfinal-tinicio,0)

    print("\n El proceso demoró " + str(tiempo), " segundos.")

    print("Fin del programa.")
elif ciclos>demasiados_ciclos:
    print("\nNo se encontraron repeticiones en " + str(demasiados_ciclos) + " repeticiones.\nPuede extender la búqueda cambiando el valor de la variable < demasiados_ciclos > en el archivo fuente del programa")
