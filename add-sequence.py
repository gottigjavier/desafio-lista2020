...
def addElements (listx3):
    return listx3[0] + listx3[1] + listx3[2]
...
while iterations_no_def or iterations > iterations_max_control: # The second condition is a protection in case no repetitions are found 
    iterations+=1
    suma = addElements(listx3) % 10000 # Only the last 4 digits are used
    listx3 = sortElements(listx3)
    if str(listx3) in set_listx3: # Searching of listx3 in set_listx3
        iterations_no_def=False
    else:
...
if not iterations_no_def:
    ...
    print("\nThe last four digits of the sum in the position " + str(last_position) + " are: ", suma)
